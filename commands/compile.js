...
async function compile(projectName, projectPath) {
    ...
    try {
        ...
        const projectDestDir = path.join(workspaceDir, path.basename(absoluteProjectPath));
        fs.cpSync(absoluteProjectPath, projectDestDir, { recursive: true });
        ...
        try {
            ...
            // Check if .u file was created despite the error
            const compiledFile = path.join(workspaceSystemDir, `${projectName}.u`);
            if (fs.existsSync(compiledFile)) {
                console.warn('Warning: ucc make returned an error, but the compiled .u file was found. Continuing...');
            } else {
                console.error('ucc make failed and no compiled .u file was found');
        ...
        if (fs.existsSync(compiledFile)) {
            ...
            if (strip) {
                ...
                try {
                    ...
                    console.error('Stripping source code failed');
                    process.exit(error.status || 1);
                }
